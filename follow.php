...
$list_follow=$class_index->list_follow($conn,$user_info['user_id'],$page,$limit);
$list_tag_footer=$class_index->list_tag_footer($conn,10);
$thongke = mysqli_query($conn, "SELECT phim.* FROM follow INNER JOIN phim ON follow.phim=phim.id WHERE follow.user_id='{$user_info['user_id']}' ORDER BY follow.date_post DESC");
$total_phim=mysqli_num_rows($thongke);
$total_page=ceil($total_phim/$limit);
$phantrang=$class_index->phantrang($page,$total_page,'/theo-doi.html');
$replace=array(
	'header'=>$skin->skin_normal('skin/header'),
	'navbar'=>$navbar,
	'box_top'=>$skin->skin_normal('skin/box_top'),
	'footer'=>$skin->skin_normal('skin/footer'),
	'title'=>'Phim đang theo dõi - '.$index_setting['title'],
...
echo $skin->skin_replace('skin/follow',$replace);
?>