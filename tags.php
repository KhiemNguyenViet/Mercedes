...
$keywords=addslashes(strip_tags($_REQUEST['keyword']));
...
if(strpos($keywords, ' ')!==false){
	...
	$thongke = mysqli_query($conn, "SELECT * FROM phim WHERE tags LIKE '%$keywords%' ORDER BY id ASC");	
}
$total_phim=mysqli_num_rows($thongke);
$total_page=ceil($total_phim/$limit);
$phantrang=$class_index->phantrang($page,$total_page,'/tags/'.urlencode($keywords).'.html');
$list_phim=$class_index->list_phim_tags($conn,$keywords,$page,$limit);
...
$replace=array(
	...
	'title'=>'Tags: '.$keywords.' - '.$index_setting['title'].''.$title_page,
...
echo $skin->skin_replace('skin/tags',$replace);
?>